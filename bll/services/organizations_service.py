...
from dal.db_models import Organizations
from dal.interfaces.organizations_interfaces import IOrganizationsRepository

from ..exceptions import EntityNotFoundError
from ..interfaces.organizations_interface import IOrganizationsService
...
class OrganizationsService(IOrganizationsService):
    def __init__(self, repository: IOrganizationsRepository):
    ...
    async def get_by_id(self, id):
        ...
        if not obj_to_get:
            raise EntityNotFoundError(f"Organization with ID {id} not found")
    ...
    async def create(self, data: OrganizationRequest):
        obj_to_create = Organizations(
            name=data.name,
            api_key=data.api_key,
        )
        return await self.repository.create(obj_to_create)

    async def update(self, id, data: OrganizationRequest):
